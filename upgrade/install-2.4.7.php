...
function upgrade_module_2_4_7($object)
{
    ...
    if (empty($firmlet_version)) {
        ...
        $firmlet_version = 'before 2.2.7';
    }

    $success = $object->removeOverride('Order') &&
        $object->addOverride('Order') &&
        Configuration::updateValue(Tools::strtoupper('VOSFACTURES_' . 'version'), '2.4.7');

    $object->addLog(sprintf('Upgrade from %s to 2.4.7: %s', $firmlet_version, $success ? 'success' : 'failure'));
